            ...
            <div className="consultancy-service-list">
              {dictionary.services.map((category: any, index: number) => (
                <div key={index} style={{ breakInside: "avoid", pageBreakInside: "avoid", marginBottom: "2.5rem" }}>
                  <h4 style={{ fontSize: "1.25rem", fontWeight: 700, color: "#bd8c2e", marginBottom: "1rem", fontFamily: '"Baloo Thambi 2", system-ui' }}>
                    {category.title}
                  </h4>
                  {category.subtitle && (
                    <p style={{ fontSize: "0.8rem", color: "#666", marginTop: "-0.75rem", marginBottom: "1rem" }}>
                      {category.subtitle}
                    </p>
                  )}
                  <ul>
